...
class System:
    ...
    thiol radical (type 2) adds to ene group (type 1) preferentially
    >-<* + o-o = >-<o*-o
    0-2  + 1-1 = 0-0-3-1
    radical thiols (2) add to ene bond (1) that has only one bond
    '''
    def bond_formation(self,snapshot,box,positions,particle_ids,ids,idx):
        ...
        if self.thiol_reaction_probability>0:
            ...
            # the ene groups that are candidates only have one bond and their positions
            ...
            if len(radicals_ids)>0 and len(polymer_candidates_ids)>0:
                ...
                print(actual_ids_polymers)
                print(actual_ids_radicals)
                print(actual_ids)
                ...
                for a in np.unique(actual_ids_radicals):
                    ...
                    if len(neigh_a)>0 and react <= self.thiol_reaction_probability:  
                        ...
                        print(b)
                        print(actual_ids)
                        actual_ids = np.vstack([i for i in actual_ids if not i[1]==b])
                        print(actual_ids)
