...
class bulk_polymerize(hoomd.custom.Action):
    '''
    This class is a larger combination of all the polymerization reaction actions, so
        that the custom action only needs to get and set a snapshot once
    '''
    def __init__(self,r_cut,simple_system,simulator):
        ## get the simulation/job
        ...
        self.chain_side_reaction_probability = simulator.job.sp["chain_side_reaction_probability"]
        self.chain_transfer_probability = simulator.job.sp["chain_transfer_probability"]
        self.thiol_reaction_probability = simulator.job.sp["thiol_reaction_probability"]
        try:
            self.termination_probability = simulator.job.sp["termination_probability"]
        except:
    ...
    def act(self,timestep):
        ...
        '''
        thiol-ene propagation reaction: thiol radicals add terminally to ene group
        ...
        '''
        args: self, snapshot, box, positions, particle_ids, ids, idx
        chain growth of ene carbons: a ene radical propagates from carbon to carbon double bond
        ...
        '''
        if self.termination_probability > 0:
            # Read the simulation state
            '''
            ids = np.arange(len(snapshot.particles.tag))
            idx = snapshot.particles.rtag[ids]
        ...
        print("polymerization_logic: ",start - polymerization_start)
        start = timer()
...
class bond_formation(hoomd.custom.Action):
    ...
    0-2  + 1=1-1=1 = 0-0-3-1-1=1
    '''
    def __init__(self,probability,r_cut,simple_system):
        ## define chemical tags ----
        self.particles_types = ['Thiol','Ene_C','RSulfur','RCarbon', 'Sulfur', 'Carbon','C','D','Dummy']
        ## ---------------[  0  , 1  ,     2     ,  3 ,     4     ,   5]
        self.bond_types = ['-SH','New','CC_Double','-S-','CC_Single','Dummy']
