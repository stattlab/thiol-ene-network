...
class Simulator():
    ...
    def polymerize(self):
        ...
        for i in range(10000):
            ...
            period_times.append(period_end - propagate_start)

            gsd_writer.flush()

            if (period_end - simulation_start) > 30*60:
                print("average_polymerization_time: ", np.average(polymerization_times))
                print("average_period_time: ", np.average(period_times))
                self.job.doc['average_polymerization_time'] = np.average(polymerization_times)
                self.job.doc['average_period_time'] = np.average(period_times)
                exit()
    ...
    #     if FJ_system==False:
    #         cosinesq = hoomd.md.angle.CosineSquared()
    #         cosinesq.params[S.angle_types] = dict(k=self.angle_constant,    t0=np.pi)
    #         cosinesq.params['Dummy'] = dict(k=1e-6, t0=np.pi)  # k>0 to make warning go away (should not do anything)
