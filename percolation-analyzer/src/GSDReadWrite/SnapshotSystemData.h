...
struct SnapshotSystemData {
    ...
    double3 global_box;                     //!< The dimensions of the simulation box
    std::vector< float3 > pos;             //!< positions
    std::vector< double3 > vel;             //!< velocities
    std::vector< float > mass;             //!< velocities
    std::vector<unsigned int> type;        //!< types
    std::vector<double3> image;             //!< images
    std::vector<int> bond_type;            //!< bond types
    std::vector<int2> bond_group;          //!< bond group
