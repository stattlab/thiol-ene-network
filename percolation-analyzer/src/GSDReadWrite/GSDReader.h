...
/*! Read an input GSD file and generate a system snapshot. GSDReader can read any frame from a GSD
    ...
    Reading and writing a gsd snapshot looks like this:

    auto reader = std::make_shared<GSDReader>(filename, frame_number, false);
    std::shared_ptr< SnapshotSystemData<float> > snap = reader->getSnapshot();

    auto writer = std::make_shared<GSDDumpWriter>(output_filename,true,false);
    writer->analyze(frame_number,*snap);
